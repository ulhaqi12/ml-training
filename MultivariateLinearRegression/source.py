...
class Model:
    ...
    def predict(self, x):
        ...
        y = np.matmul(x.to_numpy(), self.w).reshape(545,)
        return y
    ...
    def optimize(self, x, y, outputs, learning_rate=0.05):
        ...
        self.w = self.w - (learning_rate * (np.matmul(x.to_numpy().T, outputs - y)/len(y))).reshape(14, 1)
...
def plot_line_2d_using_pca(x, y, model):
    """
    funciton that will convert all data and weights in 2d using PCA and plot to get a 2d visualization.
    ...
    new_df = [x, pd.DataFrame(model.w.reshape(1, 14), columns=x.columns)]
    new_df = pd.concat(new_df, ignore_index=True)

    pca_weights = pca.fit_transform(new_df)[545]
